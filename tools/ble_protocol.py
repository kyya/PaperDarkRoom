...
Trimmed copy of the companion dashboard's daemon/ble_protocol.py, kept in sync
BY HAND (not import) since this is a separate repo. Only the constants
tools/ble_ota.py actually needs are here — the game firmware only ever registers
OTA/DATA/STAT (+ the always-present SERVICE/CTRL/TAP_ACK/TIME_CONFIG
characteristics it inherits from the reused ble_link.cpp, kept for cross-flash
compatibility). The authoritative definition lives in the firmware:
src/ble_link.cpp.
